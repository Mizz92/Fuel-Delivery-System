...
def user_order():
    ...
    mycursor.execute("INSERT INTO order_detail (user_id, gas_id, quantity, status, date, Payment_Option) VALUES (%s,%s,%s,%s,%s,%s)",(user_id, gas_id, gas_quant, gas_status, formatted_date,Payment_Option))
    mydb.commit()
...
def user_order_all():
    mycursor.execute("SELECT gas_id, quantity, date, status, Payment_Option FROM order_detail where user_id=%s",(client_user,))
    myresult = mycursor.fetchall()
    print(list(myresult))
